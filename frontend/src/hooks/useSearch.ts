...
export const useSearch = () => {
  ...
  const getUrlParams = useCallback(() => {
    ...
    return {
      ...
      isHybridSearch: params.get('mode') ? params.get('mode') === 'hybrid' : true, // Default to hybrid if no mode specified
  ...
  const openGoogleComparison = useCallback((query: string, onError: (message: string) => void) => {
    ...
    // Date range configuration (MM/DD/YY format for Google)
    const startDate = '01/01/2016';
    const endDate = '03/31/2025';
    
    const encodedStart = encodeURIComponent(startDate);
    const encodedEnd = encodeURIComponent(endDate);
    const encodedQuery = encodeURIComponent(query);
    
    // Construct Google Images search URL with site restriction and date range
    // &udm=2 for Google Images search
    // &tbs=cdr%3A1%2Ccd_min%3A...%2Ccd_max%3A... for custom date range
    const googleUrl = `https://www.google.com/search?q=${encodedQuery}+site%3A${config.googleSearchSite}&udm=2&hl=en&tbs=cdr%3A1%2Ccd_min%3A${encodedStart}%2Ccd_max%3A${encodedEnd}`;
  ...
  const openArquivoComparison = useCallback((query: string, onError: (message: string) => void) => {
    ...
    const encodedQuery = encodeURIComponent(query);
    const arquivoUrl = `https://arquivo.pt/image/search?query=${encodedQuery}+site%3A${config.googleSearchSite}&from=20160101&to=20251021`;
